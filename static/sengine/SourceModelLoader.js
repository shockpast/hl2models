...
export default class SourceModelLoader {
	...
	load( url, onLoad, onProgress, onError ) {
		...
		const mdlpr = new Promise( ( resolve, reject ) => {
			...
			new MDLLoader( this.manager ).load( `${ url }.mdl`, resolve, onProgress, reject );

		} );

		const vvdpr = new Promise( ( resolve, reject ) => {

			new VVDLoader( this.manager ).load( `${ url }.vvd`, resolve, onProgress, reject );

		} );

		const vtxpr = new Promise( ( resolve, reject ) => {

			new VTXLoader( this.manager ).load( `${ url }.dx90.vtx`, resolve, onProgress, reject );
		...
		Promise
			...
			.then( ( [ mdl, vvd, vtx ] ) => {
				...
				let path = tokens.join( '/models/' ) + '/materials/';
